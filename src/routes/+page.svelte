...
{#if showLoadingPage }
<div transition:fade={{duration: 800}} class="bg-black/90 fixed inset-0 z-50 flex items-center justify-center">
  <div class="space-y-[10vh] text-center">
    <h1 class="text-[80px] font-semibold bg-gradient-to-r from-[#2C2933] via-[#2C2933] to-[#605D66] bg-clip-text text-transparent leading-normal">Velkommen til Zibra Group</h1>
    <div>Her er nogle tips til <span class="text-[#DE524E]">oplevelsen</span></div>
    <div class="grid gap-12 grid-cols-2 lg:grid-cols-4">
      <div class="flex flex-col items-center justify-center rounded-xl p-8 px-12 max-w-sm space-y-8 text-center bg-[#232227]/80">
    ...
    <div class="flex items-center justify-center">
      <button class="bg-[#DE524E] rounded-full text-black p-4 px-8 flex items-center space-x-4" on:click={toggleLoadingPage}>
...
<<<<<<< HEAD
<button class="fixed top-6 right-6 text-white z-40 p-3 rounded-full ring-2 ring-white cursor-pointer" on:click={toggleAudio}>
=======
<button
  class="fixed top-6 right-6 text-white z-50 p-3 rounded-full ring-2 ring-white cursor-pointer"
  on:click={toggleAudio}
>
>>>>>>> cc029c796ae344f8e5cda7d6132cad4fa5825d10
  {#if audioPlaying}
    <svg
